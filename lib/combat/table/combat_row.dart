...
import 'package:combat_tracker/combat/table/character_options_dialog.dart';
...
class _CombatRowState extends State<CombatRow> {
  ...
  Widget build(BuildContext context) {
    var row = Opacity(
      ...
      child: Container(
        decoration: BoxDecoration(
          color: _hovering
              ? Color.alphaBlend(
                  widget.hoverColor ?? ColorScheme.of(context).surfaceContainer,
                  widget.selected
          ...
          children: [
            ...
            IconButton(
              onPressed: () async {
                await showDialog(
                  context: context,
                  builder: (context) => CharacterOptionsDialog(
                    combat: widget.combat,
                    character: widget.character,
                  ),
                );
    ...
    var detector = widget.onClick == null
        ? row
        : Stack(
            children: [
              row,
    ...
    return MouseRegion(
      ...
      child: SizedBox(height: 48, child: detector),
    );
  }
}
