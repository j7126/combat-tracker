...
class DamageRecievedStats extends StatefulWidget {
  const DamageRecievedStats({super.key, required this.combat, required this.character});
          ...
          children: [
                            ...
                            Column(
                              ...
                              children: [
                                Padding(
                                  ...
                                  child: () {
                                    if (event.change > 0) {
                                      return Text("Life Gain", style: const TextStyle(fontSize: 22));
                                    }
                                    var sourceCharacter = event.source.isEmpty ? null : widget.combat.characters.firstWhereOrNull((x) => x.id == event.source);
                                    return sourceCharacter == null
                                        ? Text("Damage", style: const TextStyle(fontSize: 22))
                                        : Row(
                                            mainAxisSize: MainAxisSize.min,
                                            children: [
                                              Text("Damaged by ", style: const TextStyle(fontSize: 22)),
                                              Row(
                                                mainAxisSize: MainAxisSize.min,
                                                children: [
                                                  CharacterTypeSelector(character: sourceCharacter, readOnly: true),
                                                  Text(sourceCharacter.name, style: const TextStyle(fontSize: 20)),
                                                ],
                                              ),
                                            ],
                                          );
                                ...
                                Padding(
                                  padding: const EdgeInsets.only(right: 8.0),
                                  child: Text("${TimeUtil.timeDiffString(event.timestamp.toDateTime().toLocal())} ago", style: const TextStyle(fontSize: 16)),
                                ),
                              ],
                            ),
                          ],
            ...
            Expanded(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  for (var entry in damageTotals.entries.sortedBy((x) => x.value))
                    () {
                      var sourceCharacter = widget.combat.characters.firstWhere((x) => x.id == entry.key);
                      return Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Text(entry.value.toString(), style: const TextStyle(fontSize: 22)),
                          Text(" total damage by", style: const TextStyle(fontSize: 18)),
                          Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              CharacterTypeSelector(character: sourceCharacter, readOnly: true),
